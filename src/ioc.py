...
def generate_ioc(target_url: str, forensic_report: Dict[str, Any], shap_risk_factors: List[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        'forensics': {
            ...
            'analysis_confidence': forensic_report.get('analysis_confidence', 'HIGH' if forensic_report.get('fetch_succeeded') else 'UNKNOWN'),
            'analysis_confidence_reason': forensic_report.get('analysis_confidence_reason', ''),
            'is_interstitial': forensic_report.get('is_interstitial', False),
